...
Singleton {
    ...
    Process {
        id: gpuProc
        command: ["bash", "-c", "if command -v nvidia-smi >/dev/null 2>&1; then nvidia-smi --query-gpu=utilization.gpu,temperature.gpu,memory.used,memory.total --format=csv,noheader,nounits | awk -F', ' '{printf \"%s|%s|%.1f|%.1f\\n\", $1, $2, $3/1024, $4/1024}'; else echo \"0|N/A|0|1\"; fi"]
        stdout: StdioCollector {
            onStreamFinished: {
                const parts = text.trim().split("|");
                if (parts.length === 4) {
