...
Item {
    ...
    property var _theme: Appearance.palette
    readonly property string uiFontFamily: Appearance.font.family
    readonly property string wallpaperScriptPath: Quickshell.env("HOME") + "/.config/hypr/scripts/wall"

    function applyWallpaperPath(path) {
        if (!path) return;

        Quickshell.execDetached([window.wallpaperScriptPath, path]);
        window.closeRequested();
    }

    function syncLocalThumbnails() {
        const thumbPath = decodeURIComponent(window.thumbDir.replace(/^file:\/\//, ""));
        const syncScript = `
            ...
            for img in "$SRC_DIR"/*; do
                ...
                ext=$(printf '%s\n' "$filename" | awk -F. '{print tolower($NF)}')
                ...
                case "$ext" in
                    ...
                    jpg|jpeg|png|webp|gif)
                        ...
                        if command -v magick >/dev/null 2>&1; then
                            magick "$img" -resize x420 -quality 70 "$thumb" >/dev/null 2>&1
                        else
                            cp "$img" "$thumb"
                        fi
                        ;;
        ...
        Quickshell.execDetached(["bash", "-c", syncScript]);
    }
    ...
    readonly property string srcDir: Quickshell.env("HOME") + "/Documents/Wallpapers"
    ...
    FolderListModel {
        ...
        onCountChanged: {
            if (window.isDownloadingWallpaper && window.isDownloaded(window.currentDownloadName)) {
                ...
                Qt.callLater(window.triggerColorExtraction);
            }
        }
    }
        ...
        delegate: Item {
            ...
            function pickWallpaper() {
                ...
                if (window.currentFilter === "Search" && window.hasSearched) {
                    ...
                    if (alreadyExists) {
                        window.applyWallpaperPath(destFile);
                    } else {
                        ...
                        const downloadScript = `
                            ...
                            export APPLY_SCRIPT="${escapeBash(window.wallpaperScriptPath)}"
                            export MAP_FILE="${escapeBash(mapFile)}"
                            export THUMB_DIR="${escapeBash(decodeURIComponent(window.thumbDir.replace("file://", "")))}"
                            ...
                            (
                                ...
                                if [ -n "$URL" ]; then
                                    curl -fsSL -A "Mozilla/5.0" "$URL" -o "$DEST_FILE.tmp" || exit 0
                                    
                                    if file "$DEST_FILE.tmp" | grep -iq "webp"; then
                                        if command -v magick >/dev/null 2>&1 && magick "$DEST_FILE.tmp" "$DEST_FILE"; then
                                            rm -f "$DEST_FILE.tmp"
                                        else
                                    ...
                                    if command -v magick >/dev/null 2>&1; then
                                        magick "$DEST_FILE" -resize x420 -quality 70 "$THUMB_DIR/$SAFE_NAME" >/dev/null 2>&1
                                    else
                                        cp "$DEST_FILE" "$THUMB_DIR/$SAFE_NAME"
                                    fi
                ...
                window.applyWallpaperPath(originalFile)
            }
    ...
    Component.onCompleted: {
        Quickshell.execDetached(["bash", "-c", "mkdir -p '" + decodeURIComponent(window.searchDir.replace("file://", "")) + "' '" + decodeURIComponent(window.thumbDir.replace("file://", "")) + "' '" + window.srcDir + "'"]);
