...
Item {
    ...
    Settings {
        id: cache
        property string lastEthernetIface: ""
        property string lastEthernetJson: ""
    ...
    readonly property string scriptsDir: Quickshell.env("HOME") + "/.config/quickshell/network_desktop/scripts"
    ...
    readonly property color ethernetAccent: Qt.lighter(window.sapphire, 1.15) 
    readonly property color btAccent: window.mauve

    property string activeMode: "bt"
    readonly property color activeColor: activeMode === "ethernet" ? window.ethernetAccent : window.btAccent
    ...
    Timer { id: ethernetPendingReset; interval: 8000; onTriggered: { window.ethernetPowerPending = false; window.expectedEthernetPower = ""; } }
    ...
    function syncCores() {
        let list = activeMode === "ethernet" ? (isEthernetConn && ethernetConnected ? [window.ethernetConnected] : []) : window.btConnected;
        ...
        for (let i = 0; i < list.length && i < 5; i++) {
            let dev = list[i];
            let id = window.activeMode === "ethernet" ? dev.id : dev.mac;
            for (let c = 0; c < 5; c++) {
                if (newCores[c] && (window.activeMode === "ethernet" ? newCores[c].id : newCores[c].mac) === id) { 
        ...
        for (let i = 0; i < list.length && i < 5; i++) {
            ...
            let isFound = false;
            for (let c = 0; c < 5; c++) {
                if (newCores[c] && (window.activeMode === "ethernet" ? newCores[c].id : newCores[c].mac) === id) { isFound = true; break; }
            }
            if (!isFound) {
                for (let c = 0; c < 5; c++) {
    ...
    function syncModel(listModel, dataArray) {
        ...
        for (let i = 0; i < dataArray.length && i < 30; i++) {
            ...
            let obj = {
                id: d.id || "", mac: d.mac || "",
                name: d.name || d.id || "", icon: d.icon || "", security: d.security || "", action: d.action || "",
    ...
    property var nextEthernetList: null
    property var nextBtList: null
    property var nextInfoList: null

    onIsListLockedChanged: {
        if (!isListLocked) {
            if (nextEthernetList !== null) { window.syncModel(ethernetListModel, nextEthernetList); window.ethernetList = nextEthernetList; nextEthernetList = null; }
    ...
    property bool ethernetPowerPending: false
    property string expectedEthernetPower: ""
    property string ethernetPower: "off"
    property var ethernetConnected: null
    property var ethernetList: []
    property string strongestEthernetIface: ""
    readonly property bool isEthernetConn: !!window.ethernetConnected && window.ethernetConnected.id !== undefined

    readonly property string targetEthernetIface: {
        let found = false;
        if (cache.lastEthernetIface !== "") {
            for (let i = 0; i < ethernetList.length; i++) {
                if (ethernetList[i].id === cache.lastEthernetIface) { found = true; break; }
            }
        }
        return found ? cache.lastEthernetIface : strongestEthernetIface;
    }

    onEthernetConnectedChanged: {
        if (window.ethernetConnected && window.ethernetConnected.id) { cache.lastEthernetIface = window.ethernetConnected.id; }
        syncCores();
        if (window.currentConn && window.activeMode === "ethernet") updateInfoNodes();
    }
    ...
    readonly property bool currentPowerPending: activeMode === "ethernet" ? window.ethernetPowerPending : window.btPowerPending
    readonly property bool currentConn: activeMode === "ethernet" ? window.isEthernetConn : window.isBtConn
    
    readonly property var currentObjList: activeMode === "ethernet" ? (window.isEthernetConn ? [window.ethernetConnected] : []) : window.btConnected
    ...
    function updateInfoNodes() {
        ...
        let wConn = window.ethernetConnected;
        if (Array.isArray(wConn)) wConn = wConn[0]; 
        let cList = window.activeMode === "ethernet" ? (window.isEthernetConn && wConn ? [wConn] : []) : window.btConnected;
    ...
    function processEthernetJson(textData) {
        if (textData === "") return;
        try {
            let data = JSON.parse(textData);
            let fetchedPower = data.power || "off";
            
            if (window.ethernetPowerPending) {
                window.ethernetPower = window.expectedEthernetPower; 
                if (fetchedPower === window.expectedEthernetPower) {
                    window.ethernetPowerPending = false; 
                    ethernetPendingReset.stop();
                }
            } else {
                window.ethernetPower = fetchedPower;
                window.expectedEthernetPower = "";
            }

            let wasEthernetConn = window.isEthernetConn;
            let newConnected = data.connected;
            if (JSON.stringify(window.ethernetConnected) !== JSON.stringify(newConnected)) {
                window.ethernetConnected = newConnected;
            }
            ...
            if (newNetworks.length > 0) {
                ...
                window.strongestEthernetIface = bestSsid;
            ...
            if (JSON.stringify(window.ethernetList) !== JSON.stringify(newNetworks)) {
                ...
                else { window.syncModel(ethernetListModel, newNetworks); window.ethernetList = newNetworks; window.nextEthernetList = null; }
            }

            if (window.activeMode === "ethernet") {
                if (!wasEthernetConn && window.isEthernetConn) {
                ...
                let ddChanged = false;
                for (let ssid in dd) {
                    if (!window.isEthernetConn || (window.ethernetConnected && window.ethernetConnected.id !== ssid)) {
                        delete dd[ssid];
                        ddChanged = true;
                    }
                }
                ...
                if (window.isEthernetConn && window.ethernetConnected && bt[window.ethernetConnected.id]) {
                    newlyConnected = true;
                    delete bt[window.ethernetConnected.id];
                }
                if (newlyConnected) {
    ...
    function processBtJson(textData) {
        ...
        try {
            ...
            if (window.isBtConn && window.activeMode === "bt") {
                newDevices.push({ id: "action_settings", mac: "action_settings", name: "Current Device", icon: "󰒓", action: "View Info", isInfoNode: false, isActionable: true, cmdStr: "TOGGLE_VIEW", parentIndex: -1 });
            }
    ...
    Process {
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                cache.lastEthernetJson = this.text.trim();
                processEthernetJson(cache.lastEthernetJson);
            }
        }
    }
    ...
    Timer {
        ...
        onTriggered: { 
            if (!ethernetPoller.running) ethernetPoller.running = true; 
        ...
        Rectangle {
            ...
            Item {
                ...
                Repeater {
                    ...
                    delegate: Item {
                        ...
                        property real multiShift: window.activeMode === "ethernet" ? 0.0 : window.multiTransitionState
                        ...
                        property string myId: myDevice ? (window.activeMode === "ethernet" ? myDevice.id : myDevice.mac) : "unknown"
                        ...
                        Rectangle {
                            ...
                            ColumnLayout {
                                ...
                                Text {
                                    ...
                                    text: window.activeMode === "ethernet" ? "󰈀" : "󰂲"
                                }
                                Text {
                                    Layout.alignment: Qt.AlignHCenter
                                    ...
                                    text: window.currentPowerPending 
                                        ? ((window.activeMode === "ethernet" ? window.expectedEthernetPower : window.expectedBtPower) === "on" ? "Powering On..." : "Powering Off...") 
                            ...
                            Item {
                                ...
                                ColumnLayout {
                                    ...
                                    Text {
                                        ...
                                        text: isMyDisconnecting ? "" : (coreMa.containsMouse ? (window.activeMode === "ethernet" ? "󰈀" : "󰂲") : (coreContainer.myDevice ? coreContainer.myDevice.icon : ""))
                                        Behavior on color { ColorAnimation { duration: 200 } }
                                    }
                                    ...
                                    Text {
                                        ...
                                        color: isMyDisconnecting ? window.overlay1 : window.crust
                                        text: coreContainer.myDevice ? (window.activeMode === "ethernet" ? coreContainer.myDevice.id : coreContainer.myDevice.name) : ""
                            ...
                            NumberAnimation {
                                ...
                                onFinished: {
                                    ...
                                    let cmd = window.activeMode === "ethernet" 
                                    ...
                                    if (window.activeMode === "ethernet") ethernetPoller.running = true; else btPoller.running = true;
                                }
                            }
                ...
                Item {
                    ...
                    Repeater {
                        id: orbitRepeater
                        model: (window.currentConn && window.showInfoView) ? infoListModel : (window.activeMode === "ethernet" ? ethernetListModel : btListModel)
                        ...
                        delegate: Item {
                            ...
                            property real unifiedRatio: window.activeMode === "ethernet" ? 0.0 : window.multiTransitionState
                            ...
                            property real safeMultiShift: window.activeMode === "ethernet" ? 0.0 : window.multiTransitionState
                            ...
                            Rectangle {
                                ...
                                property bool isTargetEthernet: window.activeMode === "ethernet" && !window.isEthernetConn && itemId === window.targetEthernetIface
                                property bool isSpecialAction: itemId === "action_scan" || itemId === "action_settings"
                                property bool isHighlighted: isPairedBT || isTargetEthernet || isSpecialAction
                                
                                property bool isCurrentlyConnected: {
                                    if (window.activeMode === "ethernet") return (window.ethernetConnected && window.ethernetConnected.id === itemId);
                                    for (let i = 0; i < window.btConnected.length; i++) {
            ...
            Rectangle {
                ...
                RowLayout {
                    ...
                    Rectangle {
                        ...
                        color: window.activeMode === "ethernet" ? "transparent" : (ethernetTabMa.containsMouse ? window.surface1 : "transparent")
                        Behavior on color { ColorAnimation { duration: 200 } }

                        Rectangle {
                            anchors.fill: parent
                            radius: 10
                            opacity: window.activeMode === "ethernet" ? 1.0 : 0.0
                            Behavior on opacity { NumberAnimation { duration: 300 } }
                            gradient: Gradient {
                                orientation: Gradient.Horizontal
                                GradientStop { position: 0.0; color: Qt.lighter(window.ethernetAccent, 1.15) }
                                GradientStop { position: 1.0; color: window.ethernetAccent }
                            }
                        }
                        ...
                        RowLayout {
                            anchors.centerIn: parent
                            spacing: 8
                            Text { font.family: "Iosevka Nerd Font"; font.pixelSize: 18; color: window.activeMode === "ethernet" ? window.crust : window.text; text: "󰈀"; Behavior on color { ColorAnimation{duration:200} } }
                            Text { font.family: "JetBrains Mono"; font.weight: Font.Black; font.pixelSize: 13; color: window.activeMode === "ethernet" ? window.crust : window.text; text: "Ethernet"; Behavior on color { ColorAnimation{duration:200} } }
                        }
                        MouseArea {
                            id: ethernetTabMa; anchors.fill: parent; hoverEnabled: true; cursorShape: Qt.PointingHandCursor
                            onClicked: {
                                if (window.activeMode !== "ethernet") window.playSfx("switch.wav");
                                window.activeMode = "ethernet";
                            }
                        }
                    }
            ...
            Rectangle {
                ...
                MouseArea {
                    ...
                    onClicked: {
                        if (window.activeMode === "ethernet") {
                            if (window.ethernetPowerPending) return;
                            window.expectedEthernetPower = window.ethernetPower === "on" ? "off" : "on";
                            window.ethernetPowerPending = true;
                            
                            if (window.expectedEthernetPower === "on") window.playSfx("power_on.wav"); else window.playSfx("power_off.wav");
                            
                            ethernetPendingReset.restart();
                            window.ethernetPower = window.expectedEthernetPower; // Optimistic
                            Quickshell.execDetached(["bash", window.scriptsDir + "/ethernet_panel_logic.sh", "--toggle"]);
                            ethernetPoller.running = true;
                        } else {
